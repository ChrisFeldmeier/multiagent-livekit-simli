...
class SilentAudioTrack(MediaStreamTrack):
    ...
    async def recv(self):
        ...
        frame = AudioFrame(
            samples=self.silence.tobytes(),
            sample_rate=self.sample_rate,
            num_channels=self.channels,
        )

        # Calculate PTS
        elapsed = self.next_frame_time - self.start_time
        frame.pts = int(elapsed / self.frame_duration)
        frame.time_base = Fraction(1, 1000)  # 1 millisecond per PTS unit
...
class BlackVideoTrack(MediaStreamTrack):
    ...
    async def recv(self):
        ...
        # Calculate PTS
        elapsed = self.next_frame_time - self.start_time
        frame.pts = int(elapsed / self.frame_interval)
        frame.time_base = Fraction(1, 1000)  # 1 millisecond per PTS unit
...
async def entrypoint(ctx: JobContext):
    ...
    ice_servers = [
        ...
        # Add more RTCIceServer instances if needed, e.g.,
    ...
    def on_track(track):
        ...
        if track.kind == "video":
            ...
            # Forward frames from aiortc track to LiveKit VideoSource
            async def forward_frames():
                logger.info("Starting forward_frames loop")
                while True:
                    try:
                        logger.debug("Start forward_frames ")

                        # Chris: here everything stops? on the .recv() file
                        frame = await relayed_video.recv()

                        logger.debug("End of forward_frames frame 1")
                        if frame:
                            logger.info(f"Video frame received from Simli: Format={frame.format}, Width={frame.width}, Height={frame.height}, PTS={frame.pts}, Time Base={frame.time_base}")
                            
                            # Check the content of the frames
                            ndarray = frame.to_ndarray(format="bgr24")
                            if ndarray is not None:
                                logger.debug(f"Frame-NDArray Shape: {ndarray.shape}")
                            else:
                                logger.warning("Received frame NDArray is None")
                
                            # Convert the aiortc VideoFrame to LiveKit VideoFrame
                            livekit_frame = rtc.VideoFrame.from_ndarray(
                                ndarray, format="bgr24"
                            )
            ...
            # Start the frame-forwarding task
            ...
            # Forward audio frames from aiortc track to LiveKit AudioSource
                ...
                while True:
                    try:
                        ...
                        if frame:
                            ...
                            # Extract samples from aiortc AudioFrame
            ...
            # Start the audio-forwarding task
            ...
            try:
                livekit_audio_track = rtc.LocalAudioTrack.create_audio_track("simli-audio", source)
    ...
    if audio_transceiver.sender:
        ...
        logger.error("Audio transceiver sender is None")

    black_video = BlackVideoTrack(width=640, height=480)
    ...
    if video_transceiver.sender:
        ...
        logger.error("Video transceiver sender is None")
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(url, json=payload, headers=headers) as response:
            ...
            if response.status == 200:
                ...
                logger.error(
                    f"Error starting audio-to-video session: {response.status} - {response_text}"
                )
                return None
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(url, json=payload, headers=headers) as response:
            ...
            if response.status == 200:
                ...
                logger.error(
                    f"Error starting WebRTC session: {response.status} - {response_text}"
                )
                return None
